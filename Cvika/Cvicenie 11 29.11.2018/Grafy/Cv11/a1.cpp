...
class GraphAsAdjacencyList {
	...
	EdgeTo& getEdge(string vertexName1, string vertexName2) {
		...
		for (EdgeTo &e: v1.edges) {
			if (e.endVertex->name == v2.name) {
				return e;
			}
		}
...
public:
	...
	bool containsEdge(string vertexName1, string vertexName2) {
		...
		catch (VertexMissingException& v) {
			return false;
		}
	...
	list<string> dfs(string startVertexName) {
		...
		stack<Vertex*>S;
		map<Vertex*, bool>is_visited;


		//vsetky vrcholy su na zaciatku nenavstivene
		for (Vertex &v : vertices) {
			is_visited[&v, false];
		}
		Vertex *vertex = &(this->getVertex(startVertexName));
		S.push(vertex);
		...
		while (!S.empty()) {
			...
			if (!is_visited[vertex]) {
				...
				lst.push_back(vertex->name);
				for (EdgeTo& e : vertex->edges) {
					S.push(e.endVertex);
				}
			}
...
int main() {
	...
	try {
		...
		make_tuple("A","C",9),make_tuple("A","D",10),make_tuple("A","F",14),make_tuple("A","H",1),
		make_tuple("A","I",3),make_tuple("B","A",7),make_tuple("C","D",8),make_tuple("D","A",2),
		make_tuple("G","A",2),make_tuple("G","F",4),make_tuple("H","D",5),make_tuple("H","G",3),
		make_tuple("I","C",1)
		};
